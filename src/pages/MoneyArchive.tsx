...
const fetchDataForDate = async (date: Date) => {
  ...
  const downloadCSV = () => {
    ...
    const csvContent = [
      "نوع البيانات,القيمة",
      `إيراد الساعات,${data.hourly_revenue}`,
      `إيراد الاشتراكات,${data.subscription_revenue}`,
      `إيراد الحجوزات,${data.reservation_revenue}`,
      `إيراد المنتجات,${data.product_revenue}`,
      `إجمالي الإيرادات,${data.total_revenue}`,
      `مصاريف الصيانة,${data.maintenance}`,
      `مصروف يومي,${data.daily_expenses || 0}`,
      `مرتبات,${data.salaries}`,
      `إجمالي المصروفات,${data.total_expenses}`,
    ...
    link.setAttribute("download", `تقرير_مالي_${format(selectedDate, 'yyyy-MM-dd')}.csv`);
    link.click();
  };
          ...
          <>
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-6">
              ...
              <RevenueCard
                ...
                icon={CalendarDays}
                value={data.reservation_revenue}
                editMode={editMode}
              ...
              <SummaryCard
                ...
                type={data.net_profit >= 0 ? "profit" : "loss"}
