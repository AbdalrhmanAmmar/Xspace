...
import { CreditCard, Search, Clock, Calendar, User, PlusCircle, X, Check, Settings } from 'lucide-react';
...
export const Subscriptions = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const totalDays = formData.type === 'أسبوعي' ? 7 : formData.type === 'نصف شهري' ? 15 : 30;
      const endDate = new Date(startDate);
      endDate.setDate(endDate.getDate() + totalDays);

      const price = formData.type === 'أسبوعي' 
        ? prices.weekly 
      ...
      const { error: subscriptionError } = await supabase
        ...
        .insert([{
          ...
          is_flexible: formData.type === 'نصف شهري' ? formData.isFlexible : false,
          created_by: user.id,
        }]);
  ...
  const filteredSubscriptions = subscriptions
    .filter(sub => sub.clientName.toLowerCase().includes(searchTerm.toLowerCase()));
      ...
      <div className="max-w-6xl mx-auto p-6">
          ...
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
            <div className="bg-slate-800 rounded-xl p-6 w-full max-w-md border border-slate-700">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-xl font-semibold text-white">إعدادات الأسعار</h2>
                <button
                  onClick={() => {
                    setShowSettings(false);
              ...
              <form onSubmit={handleSavePrices} className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setPriceFormData({ ...priceFormData, weekly: Number(e.target.value) })}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setPriceFormData({ ...priceFormData, halfMonthly: Number(e.target.value) })}
                    className="w-full px-4 py-2 bg-slate-700 border border-slate-600 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-blue-500"
                    min="0"
                    required
                  />
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setPriceFormData({ ...priceFormData, monthly: Number(e.target.value) })}
                    className="w-full px-4 py-2 bg-slate-700 border border-slate-600 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-blue-500"
                    min="0"
                    required
                  />
        ...
        {showAddForm && (
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
            <div className="bg-slate-800 rounded-xl p-6 w-full max-w-md border border-slate-700">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-xl font-semibold text-white">إضافة اشتراك جديد</h2>
                <button
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                {formData.type === 'نصف شهري' && (
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <label htmlFor="flexible" className="text-sm text-slate-300">
                ...
                <div className="bg-slate-700/50 p-4 rounded-lg">
                  <h3 className="text-white font-medium mb-2">تفاصيل الاشتراك:</h3>
                  <ul className="space-y-2 text-sm text-slate-300">
                    <li className="flex justify-between">
                      <span>عدد الأيام:</span>
                      <span>
                        {formData.type === 'أسبوعي' ? 7 : formData.type === 'نصف شهري' ? 15 : 30} يوم
                      </span>
                    </li>
                    <li className="flex justify-between">
                      <span>السعر:</span>
            ...
            >
              <div className="flex flex-col gap-4">
                <div className="flex justify-between items-start">
                  <div>
                    <h3 className="text-xl font-semibold text-white">{subscription.clientName}</h3>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <div className="flex items-center gap-2 text-slate-300">
                      ...
                      <span>من: {subscription.startDate.toLocaleDateString('ar-SA')}</span>
                    </div>
                    <div className="flex items-center gap-2 text-slate-300">
                      <Clock className="h-4 w-4" />
                      <span>إلى: {subscription.endDate.toLocaleDateString('ar-SA')}</span>
                    </div>
                  </div>
                  <div className="space-y-2">
                    <div className="flex items-center gap-2 text-slate-300">
        ...
        {filteredSubscriptions.length === 0 && (
          <div className="text-center py-12">
            <p className="text-xl text-slate-400">لا توجد اشتراكات مطابقة للبحث</p>
          </div>
        )}
      </div>
    </div>
