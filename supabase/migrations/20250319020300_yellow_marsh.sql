...
DROP TABLE IF EXISTS public.subscription_prices CASCADE;
DROP TABLE IF EXISTS public.hall_prices CASCADE;
...
CREATE TABLE public.reservations (
  ...
  created_at timestamptz DEFAULT now(),
  updated_at timestamptz DEFAULT now(),
  CONSTRAINT valid_hall_name CHECK (hall_name IN ('القاعة الكبيرة', 'القاعة الصغيرة'))
);
